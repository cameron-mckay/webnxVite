...
import { getAssetByID, getAssetHistory } from '../../plugins/dbCommands/assetManager';
import { getPartByID } from '../../plugins/dbCommands/partManager';
import { getUserByID } from '../../plugins/dbCommands/userManager';
import { AssetEvent, AssetSchema, CartItem, PartSchema, User, UserState } from '../../plugins/interfaces';
...
async function checkForAsset(historyEvent: keyedEvent) {
    ...
    if(!assets.value.has(historyEvent.asset_id)){
        getAssetByID(http, historyEvent.asset_id, (data, err) => {
            ...
            checkUser(temp)
            if(historyEvent.info_updated&&(temp.prev!=null)&&!assets.value.has(temp.prev!)) {
...
<template>
    <div>
        ...
        <AssetEventComponent :assets="assets" :users="users" :parts="parts" :event="event" :key="event.key" v-for="event in history"/>
    </div>
</template>
