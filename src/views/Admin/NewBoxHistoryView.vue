...
import { getNewBoxes } from '../../plugins/dbCommands/userManager';
import {
  AnalyticsSearchPage,
  BoxEvent,
  UserState,
  ...
  analyticsSearchObject = new AnalyticsSearch(
    (pageNum, startDate, endDate, userFilters, partFilters, hideOtherParts)=>{
      return new Promise<AnalyticsSearchPage>((res, rej)=>{
        getNewBoxes(http, startDate.getTime(), endDate.getTime(), pageNum, 10, async (data, err) => {
          if(err)
...
async function exportCSV() {
  getNewBoxes(http, analyticsSearchObject.getStartDateFromRouter().getTime(), analyticsSearchObject.getEndDateFromRouter().getTime(), 1, 10, async (data, err) => {
    if(err)
      return
    let arr = data as any[]
