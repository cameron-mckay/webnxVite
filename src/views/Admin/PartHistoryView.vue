...
function loadHistory() {
  pageCache = new Map<number, PartEvent[]>()
  pageNum.value = 1
  loadPage(1)
  // Get all users
}
...
function getPage(page: number) {
  return new Promise<{total: number, numPages: number, events: PartEvent[]}>(async (res)=>{
    // Check if page is in cache
    if(pageCache.has(page))
      return res({total: totalCheckouts.value, numPages: totalPages.value, events: pageCache.get(page)!})

    getPartCreateAndDeleteHistory(http, HTMLtoEpoch(startDate.value), HTMLtoEpoch(endDate.value), page, pageSize, async (data, err)=>{
      if(err) {
        return errorHandler("Failed to fetch history.")
      }
      let response = data as {total: number, numPages: number, events: PartEvent[]}
