...
import { getAllUsers, getUserAssetUpdates } from '../../plugins/dbCommands/userManager';
import {
  AssetEvent,
  AssetHistory,
  AssetSchema,
...
function getPage(page: number) {
  return new Promise<{ total: number, pages: number, events: AssetHistory}>((res, rej)=>{
    ...
    getUserAssetUpdates(http, user_id, startDate, endDate, page, pageSize, async (data, err) => {
      if(err)
        return rej([])
  ...
  <div>
    <div class="-mb-4 flex justify-between">
      <div>
        ...
        <h1 class="text-4xl leading-8 md:leading-10 mb-4">{{ user.first_name ? (user.first_name + " " + user.last_name + "'s ") : "" }} Asset Update History</h1>
