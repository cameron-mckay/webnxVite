...
import { dateToHTML, getTodaysDate, HTMLToDate, HTMLtoEpoch } from '../../plugins/dateFunctions';
import { getUserCheckouts } from '../../plugins/dbCommands/userManager';
import type {
UserState,
CheckInRequest,
User,
...
import { getAllUsers, getUserByID } from '../../plugins/dbCommands/userManager';
...
function getPage(page: number) {
  return new Promise<{total: number, pages: number, checkouts: CheckOutEvent[]}>(async (res)=>{
    ...
    getUserCheckouts(http, user._id!, HTMLtoEpoch(startDate.value), HTMLtoEpoch(endDate.value), page, pageSize, async (data, err)=>{
      if(err) {
...
<template>
  <div>
    ...
    <div>
      <h1 class="my-auto text-4xl">{{ user.first_name?(user.first_name + " " + user.last_name + "'s"):"" }} Check Out History</h1>
    ...
    <CheckoutHistoryComponent v-for="checkout of checkInQueue" :checkout="checkout" :kiosks="kiosks" :user="user" :parts="partsMap"/>
