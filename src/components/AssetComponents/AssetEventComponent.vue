    ...
    <div class="background-and-border p-4 my-4">
        ...
        <div class="md:grid-cols-3 grid">
            <div v-if="event.info_updated&&(asset.prev!=null)" class="detail-row col-span-1">
                ...
                <p>Asset Type:</p>
                <p v-if="asset.asset_type!=asset.asset_type"><del>{{ prevAsset.asset_type }}</del> {{ asset.asset_type }}</p>
                <p v-else>{{ prevAsset.asset_type }}</p>

                <p v-if="asset.chassis_type">Chassis Type</p>
                <p v-if="(asset.chassis_type&&prevAsset.chassis_type)&&(prevAsset.chassis_type!=asset.chassis_type)">
                <del>{{ prevAsset.chassis_type }}</del> {{ asset.chassis_type }}
                </p>
                ...
                <p v-if="(asset.manufacturer&&prevAsset.manufacturer)&&(prevAsset.manufacturer!=asset.manufacturer)">
                <del>{{ prevAsset.manufacturer }}</del> {{ asset.manufacturer }}
                </p>
                ...
                <p v-if="(asset.model&&prevAsset.model)&&(asset.model!=prevAsset.model)"><del>{{ prevAsset.model }}</del> {{ asset.model }}</p>
                <p v-else-if="asset.model">{{ asset.model }}</p>

                <p v-if="asset.serial">Serial:</p>
                <p v-if="(asset.serial&&prevAsset.serial)&&(asset.serial!=prevAsset.serial)"><del>{{ prevAsset.serial }}</del> {{ asset.serial }}</p>
                <p v-else-if="asset.serial">{{ asset.serial }}</p>

                <p v-if="asset.live != undefined">Status:</p>
                <p v-if="asset.live&&prevAsset.live||(asset.live&&(prevAsset.live==undefined))">Live</p>
                <p v-else-if="!asset.live&&!prevAsset.live||(!asset.live&&(prevAsset.live==undefined))">Inactive</p>
                <p v-else-if="!asset.live&&prevAsset.live"><del>Live</del> Inactive</p>
                <p v-else-if="asset.live&&!prevAsset.live"><del>Inactive</del> Live</p>

                <p v-if="asset.rails != undefined">Rails:</p>
                <p v-if="asset.rails&&prevAsset.rails||(asset.rails&&(prevAsset.rails==undefined))">Yes</p>
                <p v-else-if="!asset.rails&&!prevAsset.rails||(!asset.rails&&(prevAsset.rails==undefined))">No</p>
                <p v-else-if="!asset.rails&&prevAsset.rails"><del>No</del> Yes</p>
                <p v-else-if="asset.rails&&!prevAsset.rails"><del>Yes</del> No</p>

                <p v-if="asset.bay">Bay:</p>
                <p v-if="(asset.bay&&prevAsset.bay)&&(asset.bay!=prevAsset.bay)"><del>{{ prevAsset.bay }}</del> {{ asset.bay }}</p>
                <p v-else-if="asset.bay">{{ asset.bay }}</p>


                <p v-if="asset.power_port">Power Port:</p>
                <p v-if="(asset.power_port&&prevAsset.power_port)&&(asset.power_port!=prevAsset.power_port)"><del>{{ prevAsset.power_port }}</del> {{ asset.power_port }}</p>
                <p v-else-if="asset.power_port">{{ asset.power_port }}</p>

                <p v-if="asset.public_port">Public Port:</p>
                <p v-if="(asset.public_port&&prevAsset.public_port)&&(asset.public_port!=prevAsset.public_port)"><del>{{ prevAsset.public_port }}</del> {{ asset.public_port }}</p>
                <p v-else-if="asset.public_port">{{ asset.public_port }}</p>


                <p v-if="asset.private_port">Private Port:</p>
                <p v-if="(asset.private_port&&prevAsset.private_port)&&(asset.private_port!=prevAsset.private_port)"><del>{{ prevAsset.private_port }}</del> {{ asset.private_port }}</p>
                <p v-else-if="asset.private_port">{{ asset.private_port }}</p>

                <p v-if="prevAsset.ipmi_port">IPMI Port:</p>
                <p v-if="(asset.ipmi_port&&prevAsset.ipmi_port)&&(asset.ipmi_port!=prevAsset.ipmi_port)"><del>{{ prevAsset.ipmi_port }}</del> {{ asset.ipmi_port }}</p>
